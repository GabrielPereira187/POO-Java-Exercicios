 ...
 */
package com.mycompany.pessoafisjurmatrizpedido;

import Fatec.poo.model.Operacoes;
...
public class Aplic {
    public static void main(String[] args) {
        ...
        PedidoCompra pedidos[] = new PedidoCompra[3];
        ...
        while(y < 3){
            System.out.println("Digite o cpf ou cgc");
            p = entrada.next();
            resp = ope.pesquisa(pessoas, p);
            if(resp == -100){
                System.out.println("Não encontrado");
            }
            else{
                cod = entrada.nextInt();
                pedidos[y] = new PedidoCompra(cod);
                dt = entrada.next();
                valor = entrada.nextDouble();
                pedidos[y].setValorPedido(valor);
                pedidos[y].setDataPedido(dt);
                pessoas[resp].addCompras(valor);
                pessoas[resp].setPedcomp(pedidos[y]);
                pedidos[y].addPessoas(pessoas[resp]);
